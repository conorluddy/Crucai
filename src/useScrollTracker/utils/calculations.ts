 ...
 */
export function calculateVisibility(entry: IntersectionObserverEntry): VisibilityMetrics {
...
export function calculatePositions(
  ...
  const normalizedCenter = maxDistance > 0 ? Math.max(-1, Math.min(1, relativeToCenterY / maxDistance)) : 0;
  ...
  const normalizedTop = Math.max(-1, Math.min(1, relativeToTopY / (viewportHeight * 0.5)));
  ...
  const normalizedBottom = Math.max(-1, Math.min(1, relativeToBottomY / (viewportHeight * 0.5)));
...
export function calculateThresholds(
  ...
  if (thresholds.crossed.length > 0) {
    ...
    // Find next threshold
    const nextThresholds = sortedThresholds.filter(t => t > (thresholds.active || 0));
    thresholds.next = nextThresholds.length > 0 ? Math.min(...nextThresholds) : null;
  } else {
...
export function calculateDynamics(
  ...
  const inertiaDecayFactor = Math.min(1, timeSinceLastScroll / inertiaDecayTime);
 ...
 */
export function getDefaultEntryDirection(rect: DOMRectReadOnly, viewportHeight: number): 'top' | 'bottom' {
